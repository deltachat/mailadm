...
@click.option("--vmail-user", type=str, default="vmail",
              help="user under which dovecot delivers virtual mail")
@click.option("--path-virtual-mailboxes", default=None, type=str,
              help="path to postfix map that is generated by mailadm")
@click.option("--force", is_flag=True,
              help="path to postfix map that is generated by mailadm")
@click.pass_context
def init(ctx, web_endpoint, mail_domain, vmail_user, path_virtual_mailboxes, force):
    """Initialize configuration in mailadm database."""
    db = get_mailadm_db(ctx, fail_missing_config=False)
    if db.is_initialized() and not force:
        ctx.fail("database {} already initialized, mayube use --force?".format(db.path))
    db.init_config(
    ...
    click.secho("database {} initialized, use --config to view settings".format(db.path))
